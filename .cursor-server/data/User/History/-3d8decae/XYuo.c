...
void delete_hunt(char hunt_id[]) {
    static char path[PATH_SIZE], path2[PATH_SIZE], hunt_dir[PATH_SIZE];
    
    if (snprintf(hunt_dir, sizeof(hunt_dir), "hunts/%s", hunt_id) >= sizeof(hunt_dir)) {
        printf("Error: Hunt directory path too long\n");
        update_log_file(hunt_id, "--remove_hunt", "", 1);
        return;
    }
    
    if (snprintf(path, sizeof(path), "%s/treasure.bin", hunt_dir) >= sizeof(path)) {
        printf("Error: Treasure file path too long\n");
        update_log_file(hunt_id, "--remove_hunt", "", 1);
        return;
    }
    ...
    if (snprintf(path, sizeof(path), "%s/logged_hunt.txt", hunt_dir) >= sizeof(path)) {
        printf("Error: Log file path too long\n");
        update_log_file(hunt_id, "--remove_hunt", "", 1);
        return;
    }
    
    if (snprintf(path2, sizeof(path2), "hunts/logged_hunt_%s", hunt_id) >= sizeof(path2)) {
        printf("Error: Symlink path too long\n");
        update_log_file(hunt_id, "--remove_hunt", "", 1);
        return;
    }
...
int create_log_file(char dir[]) {
    static char path[PATH_SIZE], link_path[PATH_SIZE], relative_path[PATH_SIZE];
    
    if (snprintf(path, sizeof(path), "hunts/%s/logged_hunt.txt", dir) >= sizeof(path)) {
        printf("Error: Log file path too long\n");
        return 0;
    }
    
    if (snprintf(link_path, sizeof(link_path), "hunts/logged_hunt_%s", dir) >= sizeof(link_path)) {
        printf("Error: Symlink path too long\n");
        return 0;
    }
    
    // Create relative path for the symlink target
    if (snprintf(relative_path, sizeof(relative_path), "%s/logged_hunt.txt", dir) >= sizeof(relative_path)) {
        printf("Error: Relative path too long\n");
        return 0;
    }
 
    FILE *file;
    if((file = fopen(path,"w"))==NULL) {
        perror("Error at creating log file");
        return 0;
    }
    fclose(file);

    // Remove existing symlink if it exists
    unlink(link_path);  // It's okay if this fails (if link doesn't exist)

    // Create symbolic link using relative path
    if (symlink(relative_path, link_path) != 0) {
        perror("Error creating symbolic link");  // Print detailed error message
        unlink(path);  // Clean up the log file if link creation fails
        return 0;
    }
    return 1;
}
