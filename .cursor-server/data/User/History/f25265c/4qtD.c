...
void list_hunts(monitor *monitor) {
    if (monitor->status == 1)
    {
        ...
        if (file != NULL)
        {
            ...
            snprintf(command, sizeof(command), "%stm --list_hunts", get_executable_path());
            fprintf(file, "%s", command);
            printf("Debug: Writing command to file: %s\n", command);
            fclose(file);
        }
...
void calculate_score(monitor *monitor) {
    if (monitor->status == 1) {
        ...
        // Build absolute path for hunts directory
        char hunts_path[PATH_MAX];
        snprintf(hunts_path, sizeof(hunts_path), "%shunts", get_executable_path());
