...
#define COMMAND_PATH ""  // Empty for current directory, can be changed to absolute path if needed
#define TM_CMD COMMAND_PATH "tm"
#define SC_CMD COMMAND_PATH "sc"
...
void list_hunts(monitor *monitor) {
    if (monitor->status == 1)
    {
        ...
        if (file != NULL)
        {
            fprintf(file, "%s --list_hunts", TM_CMD);
            fclose(file);
        }
...
void view_treasure(monitor *monitor) {
    if (monitor->status == 1)
    {
        ...
        if(file != NULL) {
            fprintf(file, "%s --view %s %s", TM_CMD, hid, tid);
            fclose(file);
        }
...
void list_treasures(monitor *monitor) {
    if (monitor->status == 1)
    {
        ...
        if (file != NULL)
        {
            fprintf(file, "%s --list %s", TM_CMD, hid);
            fclose(file);
        }
    ...
    if (monitor->status == 1) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            if (entry->d_type == DT_DIR && strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0) {
                ...
                if (pid == 0) {
                    // Child process
                    char *args[] = {SC_CMD, entry->d_name, NULL};
                    execvp(SC_CMD, args);
                    perror("execv failed");
                    exit(1);
                } else if (pid > 0) {
