...
void list_hunts(monitor *monitor) {
    if (monitor->status == 1)
    {
        ...
        if (file != NULL)
        {
            ...
            printf("Debug: Writing command to file: %s --list_hunts\n", command);
            fclose(file);
        }

        printf("Debug: Sending SIGUSR1 to monitor process (PID: %d)\n", monitor->pid);
        if (kill(monitor->pid, SIGUSR1) == -1) {
            perror("kill failed");
        } else {
            printf("Debug: SIGUSR1 sent successfully\n");
        }
...
void start_monitor(monitor *monitor) {
    ...
    if (monitor->pid == 0) {
        ...
        // Update monitor struct for child process
        monitor->pid = getpid();
