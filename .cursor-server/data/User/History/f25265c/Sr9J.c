...
static int count_treasures(const char *hunt_path) {
    ...
    snprintf(bin_path, sizeof(bin_path), "%s/treasure.bin", hunt_path);
        ...
        if (file != NULL) {
            ...
            while ((entry = readdir(dir)) != NULL) {
                if (entry->d_type == DT_DIR && strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0) {
                    ...
                    snprintf(full_path, sizeof(full_path), "%s/%s", hunts_path, entry->d_name);
...
void read_from_pipe(monitor_t *monitor) {
    ...
    // Try to read from pipe for up to 1 second
    int attempts = 0;
    while (attempts < 10) {
