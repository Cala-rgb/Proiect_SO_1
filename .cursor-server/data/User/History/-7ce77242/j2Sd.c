...
void usr_handler(int sig) {
    ...
    if (saved_stdout == -1) {
        write(STDERR_FILENO, "failed to save stdout\n", 21);
        close(pipefd[0]);
        close(pipefd[1]);
        return;
    }
    ...
    if (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        if(pid == 0) {
            ...
            close(saved_stdout);  // Child won't need the saved stdout
            dup2(pipefd[1], STDOUT_FILENO);  // Redirect only stdout to pipe
            ...
            // Read from pipe and write to stderr
            char output[4096];
            ssize_t n;
