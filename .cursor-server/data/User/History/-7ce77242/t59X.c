...
void usr_handler(int sig) {
    write(STDERR_FILENO, "DEBUG: Entering usr_handler\n", 26);
    ...
    write(STDERR_FILENO, "DEBUG: Pipe created\n", 19);
    ...
    write(STDERR_FILENO, "DEBUG: File opened\n", 18);

    char buffer[PATH_MAX];
    if (fgets(buffer, sizeof(buffer), file) != NULL) {
        write(STDERR_FILENO, "DEBUG: Command read: ", 19);
        write(STDERR_FILENO, buffer, strlen(buffer));
        
        int pid = fork();
        if(pid == 0) {
            // Child process
            write(STDERR_FILENO, "DEBUG: In child process\n", 22);
            close(pipefd[0]);  // Close read end
            dup2(pipefd[1], STDOUT_FILENO);  // Redirect stdout to pipe
            dup2(pipefd[1], STDERR_FILENO);  // Also redirect stderr to pipe
            close(pipefd[1]);
            ...
            // Parent process
            write(STDERR_FILENO, "DEBUG: In parent process\n", 23);
            close(pipefd[1]);  // Close write end
            
            // Read and forward the output through the monitor's pipe
            char output[4096];
            ssize_t n;
            while ((n = read(pipefd[0], output, sizeof(output)-1)) > 0) {
                write(STDERR_FILENO, "DEBUG: Read from pipe: ", 21);
                write(STDERR_FILENO, output, n);
            ...
            char status_msg[100];
            snprintf(status_msg, sizeof(status_msg), "DEBUG: Command completed with status %d\n", WEXITSTATUS(status));
    ...
    fclose(file);
    write(STDERR_FILENO, "DEBUG: Handler completed\n", 23);
}
