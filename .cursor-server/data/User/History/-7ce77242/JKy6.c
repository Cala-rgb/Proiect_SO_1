...
void usr_handler(int sig) {
    printf("Debug: Entering usr_handler in process %d\n", getpid());
    FILE *file;
    ...
    if (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        if(pid == 0) {
            // Child process
            printf("Debug: Executing command in child process\n");
...
void handle_usr() {
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));  // Initialize all fields to 0
    sa.sa_handler = usr_handler;
    sigemptyset(&sa.sa_mask);
    sa.sa_flags = SA_RESTART;  // Add SA_RESTART flag
    ...
    printf("Debug: SIGUSR1 handler set successfully\n");
}