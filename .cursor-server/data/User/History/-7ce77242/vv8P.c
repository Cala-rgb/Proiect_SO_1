...
void usr_handler(int sig) {
    ...
    if (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        if(pid == 0) {
            ...
            // Make sure child's stdout is unbuffered
            setvbuf(stdout, NULL, _IONBF, 0);
            ...
            // Create a FILE* from the pipe for buffered I/O
            FILE* pipe_fp = fdopen(pipefd[0], "r");
            if (pipe_fp) {
                char output[4096];
                size_t n;
                while ((n = fread(output, 1, sizeof(output)-1, pipe_fp)) > 0) {
                    fwrite(output, 1, n, stdout);
                    fflush(stdout);  // Force flush after each write
                }
                fclose(pipe_fp);  // This will also close pipefd[0]
            } else {
                close(pipefd[0]);
            }
