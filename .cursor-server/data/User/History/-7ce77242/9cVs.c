...
void usr_handler(int sig) {
    ...
    snprintf(command_file, sizeof(command_file), "%s/given_command.txt", get_working_dir());
    ...
    if (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        if(pid == 0) {
            // Child process
            // Change to the working directory before executing command
            if (chdir(get_working_dir()) != 0) {
                perror("chdir failed");
                exit(1);
            }
