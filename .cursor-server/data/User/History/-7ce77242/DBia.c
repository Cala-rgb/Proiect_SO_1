...
void usr_handler(int sig) {
    ...
    if (len + strlen("/given_command.txt") + 1 > sizeof(command_file)) {
        fprintf(stderr, "Path too long for command file\n");
        return;
    }
    strcpy(command_file, current_dir);
    strcat(command_file, "/given_command.txt");
