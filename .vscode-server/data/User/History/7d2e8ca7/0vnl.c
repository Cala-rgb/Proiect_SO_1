...
void updateLogFile(char hunt[], char op[], char tid[], int failed) {
    ...
    if(failed==1) {
        if(strcmp(op,"--list")==0) {
            strcpy(msg,"Listed hunt ");
            strcpy(msg,hunt);
            strcpy(msg," -> failed!");
        }
        else if(strcmp(op,"--add")==0) {
            strcpy(msg,"Added treasure ");
            strcpy(msg,tid);
            strcpy(msg," to hunt ");
            strcpy(msg,hunt);
            strcpy(msg," -> failed!");
...
void createTreasure(char *hunt)
{
    ...
    if(write(file, &t, sizeof(treasure)) != sizeof(treasure)) {
        ...
        updateLogFile(argv[2],"--add",t.id,1);
    }

    updateLogFile(argv[2],"--add",t.id,0);
...
void view_treasure(char hunt_id[], char treasure_id[]) {
    ...
    if(read(file,&t,sizeof(treasure))!= sizeof(treasure)) {
        ...
        updateLogFile(argv[2],"--view_treasure",treasure_id,1);
    } else {
    ...
    updateLogFile(argv[2],"--view_treasure",treasure_id,0);
}
...
void delete_dir(char hunt_id[]) {
    ...
    snprintf(path2, sizeof(path),"%s%s%s","logged_hunt_",dir,".txt");
    ...
    updateLogFile(argv[2],"--remove_hunt","",0);
...
void delete_treasure(char hunt_id[], char treasure_id[]) {
    ...
    if(file = open(path, O_RDWR, 0777)==0) {
        printf("error at opening file");
        updateLogFile(argv[2],"--remove_treasure",treasure_id,1);
    }
    ...
    updateLogFile(argv[2],"--remove_treasure",treasure_id,0);
 
}

void createBinFile(char dir[]) {
    static char path[128];
...
void createLogFile(char dir[]) {
    ...
    if((file = fopen(path,"w")) == NULL) {
        ...
        fprintf("Log file for hunt %s",dir);
        fclose(file); 
    }
